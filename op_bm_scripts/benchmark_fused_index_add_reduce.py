...
@torch.jit.script
def fused_gelu(input, dim: int, index, other):
    out = torch.index_add(input, dim, index, other)
    return torch.index_select(out, dim, index).sum(dim)


def gelu(input, dim, index, other):
    out = torch.index_add(input, dim, index, other)
    return torch.index_select(out, dim, index).sum(dim)
...
op_name = "fused_index_add_reduce"
# native_exists = True
...
for sparsity in sparsities:
    for tshape in tshapes:
        for dim in [0, 1, 2]:
            ...
            other = torch.clone(input)

            # begin benchmark logic
            t0 = benchmark.Timer(
                stmt="fused_gelu(input, dim, index, other)",
            ...
            # begin benchmark logic
            t1 = benchmark.Timer(
                stmt="gelu(input, dim, index, other)",
