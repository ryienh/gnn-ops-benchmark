...
def op_native_scatter_multiply_(src, idx):
    temp = torch.zeros_like(src)
    temp.scatter_(-1, idx, src, reduce="multiply")
    return None


# Configurable hyperparams here
setup_seed(42)
op_name = "scatter_multiply"
native_exists = True
...
for reduce_f in reduce_f_:
    for sparsity in sparsities:
        for src_dims in tshapes:
            ...
            t0 = benchmark.Timer(
                stmt="op_native_scatter_multiply_(src, idx)",
                setup="from __main__ import op_native_scatter_multiply_",
